...
%
%     Note, this function extracts phase information through FFT analysis,
%     which implicitly references a cosine basis. When analysing signals
%     created by createCWSignals (which uses a sine basis), this creates a
%     π/2 (90 degrees) phase difference between generation and analysis.
%     Specifically, a sine wave with zero phase generated by
%     createCWSignals (φ = 0) will be detected as having a phase of -π/2 by
%     this function. This should be considered when interpreting phase
%     measurements, especially when comparing with the original phase
%     values used in signal generation.
%
%     Example:
%
%         % Create a single cycle sine wave with zero phase
%         freq = 1000;          % 1 kHz
%         Fs = 1000*1000;       % 1 MHz sampling rate (plenty of samples)
%         T = 1/freq;           % Period of the wave
%         t = 0:1/Fs:T-1/Fs;    % Time array for exactly one cycle
% 
%         % Generate sine wave with zero phase using createCWSignals
%         sine_wave = createCWSignals(t, freq, 1, 0, 0);  % Amp=1, Phase=0, No ramp
% 
%         % Extract the phase using extractAmpPhase
%         [~, measured_phase] = extractAmpPhase(sine_wave, Fs, freq);
%
% USAGE:
