function test_pass = kWaveDiffusion_compare_1D_homog_perfusion_with_exact(plot_comparisons, plot_simulations) 
% DESCRIPTION:
...
blood_perfusion_rate      = 0.01;  	% [1/s] (Note: blood_density * blood_perfusion_rate = volumetric blood flow rate)

% define perfusion coefficient
medium.perfusion_coeff = blood_density .* blood_perfusion_rate .* blood_specific_heat ./ (medium.density .* medium.specific_heat);
...
T_exact = bioheatExact(source.T0, 0, [D, medium.perfusion_coeff, medium.blood_ambient_temperature], kgrid.dx, Nt*dt);
...
offset = 30e-3;
width = 3*dx;
source.Q = 1e4 * exp(-((kgrid.x-offset)/width).^2);
...
T_exact = bioheatExact(source.T0, S, [D, medium.perfusion_coeff, medium.blood_ambient_temperature], kgrid.dx, Nt*dt);
