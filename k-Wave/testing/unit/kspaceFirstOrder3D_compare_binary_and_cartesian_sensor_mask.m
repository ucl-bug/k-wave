function test_pass = kspaceFirstOrder3D_compare_binary_and_cartesian_sensor_mask(plot_comparisons, plot_simulations)
% DESCRIPTION:
...
source.p0(12, Ny/4:3*Ny/4, Nz/4:3*Nz/4) = 1;

% define Cartesian sensor points using points exactly on the grid
sphere_mask = makeSphere(Nx, Ny, Nz, Nx/2 - 10);
...
sensor_data_c_ln = kspaceFirstOrder3D(kgrid, medium, source, sensor, ...
    'PlotSim', plot_simulations);

% run the simulation using nearest-neighbour interpolation
sensor_data_c_nn = kspaceFirstOrder3D(kgrid, medium, source, sensor, ...
    'CartInterp', 'nearest', ...
    'PlotSim', plot_simulations);
...
sensor_data_b = kspaceFirstOrder3D(kgrid, medium, source, sensor, ...
    'PlotSim', plot_simulations);
