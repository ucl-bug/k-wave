function test_pass = kWaveArray_focused_annular_array_3D(plot_simulations, plot_comparisons)
%KWAVEARRAY_FOCUSED_ANNULAR_ARRAY_3D Compare with analytical solution for annular array.
%
% DESCRIPTION:
%     Comparison between the on-axis pressure from an annular array source
...
karray = kWaveArray('BLITolerance', 0.01, 'UpsamplingRate', 16, 'SinglePrecision', true);
...
% calculate error
err = 100 * max(abs(p_axial(:) - amp_on_axis(:))) / max(p_axial(:));
