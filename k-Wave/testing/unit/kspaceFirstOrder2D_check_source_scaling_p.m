function test_pass = kspaceFirstOrder2D_check_source_scaling_p(plot_comparisons, plot_simulations)
% DESCRIPTION:
%     Unit test to check the k-space source correction in
%     kspaceFirstOrder2D for a pressure source.
%
% ABOUT:
...
p_mask = zeros(Nx, Ny);
p_mask(1 + pml_size, 1 + pml_size:Ny - pml_size) = 1;
...
sensor.mask(Nx - pml_size, Ny/2) = 1;
...
% recreate the source
source_no_correction.p = createCWSignals(kgrid.t_array, freq, 1, 0);
...
if plot_comparisons
    ...
    % create time axis
    t_array_fine = (0:(length(sensor_data_fine) - 1)) * dt_fine + dt_fine/2;
    t_array_coarse = (0:(length(sensor_data_k_correction) - 1)) * dt + dt/2;
