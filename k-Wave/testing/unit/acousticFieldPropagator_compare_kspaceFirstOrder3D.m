function test_pass = acousticFieldPropagator_compare_kspaceFirstOrder3D(plot_comparisons, ~)
% DESCRIPTION:
%     Unit test to compare acousticFieldPropagator with a k-Wave simulation
%     using kspaceFirstOrder3D.
%
% ABOUT:
...
aperture(1, Ny/2 - aperture_width + 1:Ny/2 + aperture_width, Nz/2 - aperture_width + 1:Nz/2 + aperture_width) = 1;
...
amplitude_afp = squeeze(amplitude_afp(:, :, Nz/2));
phase_afp = squeeze(phase_afp(:, :, Nz/2));
amplitude_kw = squeeze(amplitude_kw(:, :, Nz/2));
phase_kw = squeeze(phase_kw(:, :, Nz/2));
