function test_pass = kspaceFirstOrder2D_check_source_scaling_uy(plot_comparisons, plot_simulations)
% DESCRIPTION:
%     Unit test to check the k-space source correction in
%     kspaceFirstOrder2D for a velocity source.
%
% ABOUT:
...
u_mask = zeros(Nx, Ny);
u_mask(1 + pml_size:Nx - pml_size, 1 + pml_size) = 1;
...
% run the simulation
sensor_data_k_correction = kspaceFirstOrder2D(kgrid, medium, source_k_correction, sensor, ...
...
% run the simulation
sensor_data_no_correction = kspaceFirstOrder2D(kgrid, medium, source_no_correction, sensor, ...
    'PlotSim', plot_simulations, ...
    'PMLSize', pml_size);
...
% run the simulation
sensor_data_w_correction = kspaceFirstOrder2D(kgrid, medium, source_w_correction, sensor, ...
    'PlotSim', plot_simulations, ...
    'PMLSize', pml_size);
...
% recreate the source
source_no_correction.uy = createCWSignals(kgrid.t_array, freq, 1, 0) / (medium.sound_speed * medium.density);

% only record the final few periods when the field is in steady state
sensor.record_start_index = kgrid.Nt - record_periods * ppp_fine + 1;

% run the simulation
sensor_data_fine = kspaceFirstOrder2D(kgrid, medium, source_no_correction, sensor, ...
    'PlotSim', plot_simulations, ...
    'PMLSize', pml_size);
