function test_pass = kspaceFirstOrder2D_compare_labelled_and_binary_source_mask(plot_comparisons, plot_simulations)
% DESCRIPTION:
...
source_3 = filterTimeSeries(kgrid, medium, source_3);
...
source.p_mask = labelled_mask;
source.p = labelled_sources;
...
sensor_data_labelled = kspaceFirstOrder2D(kgrid, medium, source, sensor, input_args{:});

% reassign the source using a binary source mask
clear source;
source.p_mask = binary_mask;
index_mask = labelled_mask(labelled_mask ~= 0);
source.p = labelled_sources(index_mask, :);

% run the simulation using the a binary source mask
sensor_data_binary = kspaceFirstOrder2D(kgrid, medium, source, sensor, input_args{:});
