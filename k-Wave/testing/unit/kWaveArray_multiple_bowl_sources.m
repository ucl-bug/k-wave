function test_pass = kWaveArray_multiple_bowl_sources(plot_comparisons, plot_simulations)
% DESCRIPTION:
...
% time array
kgrid.makeTime(medium.sound_speed, 0.5);
...
diameter  = 30e-3;
focus_pos = [0, 1, 0];
karray.addBowlElement(elem_pos, radius, diameter, focus_pos);
...
if plot_comparisons
    figure;
    imagesc(squeeze(source.p_mask(:, :, Nz/2)));
    axis image;
...
% set a spherical sensor mask
sensor.mask = makeSphere(Nx, Ny, Nz, Nx/2 - 2);
...
sensor_data_karray = kspaceFirstOrder3D(kgrid, medium, source, sensor, input_args{:});
