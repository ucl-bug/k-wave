function test_pass = kspaceFirstOrderAS_check_source_scaling_p(plot_comparisons, plot_simulations)
% DESCRIPTION:
%     Unit test to check the k-space source correction in
%     kspaceFirstOrderAS for a pressure source.
%
% ABOUT:
...
% tone burst
freq = 1e6;
source_sig_coarse = toneBurst(1/kgrid_coarse.dt, freq, 3);
source_sig_fine   = toneBurst(1/kgrid_fine.dt, freq, 3);
...
source_fine.p_mask = p_mask;
source_fine.p = source_sig_fine;

source_k_correction.p_mask = p_mask;
source_k_correction.p = source_sig_coarse;

source_w_correction.p_mask = p_mask;
source_w_correction.p = source_sig_coarse;
source_w_correction.p_frequency_ref = freq;

source_no_correction.p_mask = p_mask;
source_no_correction.p = source_sig_coarse;
...
for ind = 1:length(symmetry_array)
    ...
    sensor_data_k_us        = circshift(sensor_data_k_us, sc_fac);
    sensor_data_w_us        = circshift(sensor_data_w_us, sc_fac);
    sensor_data_no_corr_us  = circshift(sensor_data_no_corr_us, sc_fac);
    sensor_data_ref_us      = circshift(sensor_data_ref_us, 1); 
