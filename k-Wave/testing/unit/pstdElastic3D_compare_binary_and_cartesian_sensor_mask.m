function test_pass = pstdElastic3D_compare_binary_and_cartesian_sensor_mask(plot_comparisons, plot_simulations)
% DESCRIPTION:
...
medium.sound_speed_shear(Nx/2:end, :, :) = 1400;
...
sensor_data_c_ln = pstdElastic3D(kgrid, medium, source, sensor, ...
    'PMLSize', 6, ...
    'PlotSim', plot_simulations);

% run the simulation using nearest-neighbour interpolation
sensor_data_c_nn = pstdElastic3D(kgrid, medium, source, sensor, ...
    'PMLSize', 6, ...
    'CartInterp', 'nearest', ...
    'PlotSim', plot_simulations);
...
sensor_data_b = pstdElastic3D(kgrid, medium, source, sensor, ...
    'PMLSize', 6, ...
    'PlotSim', plot_simulations);
...
sensor_data_b.uz                = reorderBinarySensorData(sensor_data_b.uz, reorder_index);
...
sensor_data_b.uz_max            = reorderBinarySensorData(sensor_data_b.uz_max, reorder_index);
...
sensor_data_b.uz_min            = reorderBinarySensorData(sensor_data_b.uz_min, reorder_index);
...
sensor_data_b.uz_rms            = reorderBinarySensorData(sensor_data_b.uz_rms, reorder_index);
...
sensor_data_b.uz_non_staggered  = reorderBinarySensorData(sensor_data_b.uz_non_staggered, reorder_index);
...
sensor_data_b.Iz                = reorderBinarySensorData(sensor_data_b.Iz, reorder_index);
...
sensor_data_b.Iz_avg            = reorderBinarySensorData(sensor_data_b.Iz_avg, reorder_index);
...
err_uz_nn = max(abs(sensor_data_c_nn.uz(:) - sensor_data_b.uz(:))) / max(abs(sensor_data_b.uz(:)));
err_uz_ln = max(abs(sensor_data_c_ln.uz(:) - sensor_data_b.uz(:))) / max(abs(sensor_data_b.uz(:)));
...
err_uz_max_nn = max(abs(sensor_data_c_nn.uz_max(:) - sensor_data_b.uz_max(:))) / max(abs(sensor_data_b.uz_max(:)));
err_uz_max_ln = max(abs(sensor_data_c_ln.uz_max(:) - sensor_data_b.uz_max(:))) / max(abs(sensor_data_b.uz_max(:)));
...
err_uz_min_nn = max(abs(sensor_data_c_nn.uz_min(:) - sensor_data_b.uz_min(:))) / max(abs(sensor_data_b.uz_min(:)));
err_uz_min_ln = max(abs(sensor_data_c_ln.uz_min(:) - sensor_data_b.uz_min(:))) / max(abs(sensor_data_b.uz_min(:)));
...
err_uz_rms_nn = max(abs(sensor_data_c_nn.uz_rms(:) - sensor_data_b.uz_rms(:))) / max(abs(sensor_data_b.uz_rms(:)));
err_uz_rms_ln = max(abs(sensor_data_c_ln.uz_rms(:) - sensor_data_b.uz_rms(:))) / max(abs(sensor_data_b.uz_rms(:)));
...
err_uz_non_staggered_nn = max(abs(sensor_data_c_nn.uz_non_staggered(:) - sensor_data_b.uz_non_staggered(:))) / max(abs(sensor_data_b.uz_non_staggered(:)));
err_uz_non_staggered_ln = max(abs(sensor_data_c_ln.uz_non_staggered(:) - sensor_data_b.uz_non_staggered(:))) / max(abs(sensor_data_b.uz_non_staggered(:)));
...
err_Iz_nn = max(abs(sensor_data_c_nn.Iz(:) - sensor_data_b.Iz(:))) / max(abs(sensor_data_b.Iz(:)));
err_Iz_ln = max(abs(sensor_data_c_ln.Iz(:) - sensor_data_b.Iz(:))) / max(abs(sensor_data_b.Iz(:)));
...
err_Iz_avg_nn = max(abs(sensor_data_c_nn.Iz_avg(:) - sensor_data_b.Iz_avg(:))) / max(abs(sensor_data_b.Iz_avg(:)));
err_Iz_avg_ln = max(abs(sensor_data_c_ln.Iz_avg(:) - sensor_data_b.Iz_avg(:))) / max(abs(sensor_data_b.Iz_avg(:)));
...
if (err_p_nn > comparison_thresh) || ...
        ...
        (err_uz_nn > comparison_thresh) || ...
        (err_uz_ln > comparison_thresh) || ...
        (err_uz_max_nn > comparison_thresh) || ...
        (err_uz_max_ln > comparison_thresh) || ...
        (err_uz_min_nn > comparison_thresh) || ...
        (err_uz_min_ln > comparison_thresh) || ...
        (err_uz_rms_nn > comparison_thresh) || ...
        (err_uz_rms_ln > comparison_thresh) || ...
        (err_uz_non_staggered_nn > comparison_thresh) || ...
        (err_uz_non_staggered_ln > comparison_thresh) || ...        
        ...
        (err_Iz_nn > comparison_thresh) || ...
        (err_Iz_ln > comparison_thresh) || ...
        (err_Iz_avg_nn > comparison_thresh) || ...
