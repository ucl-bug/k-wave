function test_pass = kspaceFirstOrder_check_plane_wave_intensity_cuboid_corners(plot_comparisons, plot_simulations)
% DESCRIPTION:
%       Unit test to check the intensity of a plane wave in 1, 2, and 3D
%       using a cuboid corners sensor mask
%
% ABOUT:
...
% either version 3 of the License, or (at your option) aNj later version.
% 
% k-Wave is distributed in the hope that it will be useful, but WITHOUT ANj
...
input_args = {'PlotScale', 'auto', ...
    ...
    'PlotSim', plot_simulations, 'DisplayMask', 'off'};
...
% two cuboid sensors covering the domain
sensor.mask = [1, Nx; 1, Nx].';
...
% two cuboid sensors covering the domain
sensor.mask = [1, 1, Nx, Nj; 1, 1, Nx, Nj].';
...
% two cuboid sensors covering the domain
sensor.mask = [1, 1, Nj, Nx; 1, 1, Nj, Nx].';
...
% two cuboid sensors covering the domain
sensor.mask = [1, 1, 1, Nx, Nj, Nj; 1, 1, 1, Nx, Nj, Nj].';
...
% two cuboid sensors covering the domain
sensor.mask = [1, 1, 1, Nj, Nx, Nj; 1, 1, 1, Nj, Nx, Nj].';
...
% two cuboid sensors covering the domain
sensor.mask = [1, 1, 1, Nj, Nj, Nx; 1, 1, 1, Nj, Nj, Nx].';
...
sd_1D_x.Ix_avg = squeeze(sensor_data_1D_x(1).Ix_avg(sensor_position));
sd_2D_x.Ix_avg = squeeze(sensor_data_2D_x(1).Ix_avg(sensor_position, end/2));
sd_2D_y.Iy_avg = squeeze(sensor_data_2D_y(1).Iy_avg(end/2, sensor_position));
sd_3D_x.Ix_avg = squeeze(sensor_data_3D_x(1).Ix_avg(sensor_position, end/2, end/2));
sd_3D_y.Iy_avg = squeeze(sensor_data_3D_y(1).Iy_avg(end/2, sensor_position, end/2));
sd_3D_z.Iz_avg = squeeze(sensor_data_3D_z(1).Iz_avg(end/2, end/2, sensor_position));

sd_1D_x.Ix = squeeze(sensor_data_1D_x(1).Ix(sensor_position, :));
sd_2D_x.Ix = squeeze(sensor_data_2D_x(1).Ix(sensor_position, end/2, :));
sd_2D_y.Iy = squeeze(sensor_data_2D_y(1).Iy(end/2, sensor_position, :));
sd_3D_x.Ix = squeeze(sensor_data_3D_x(1).Ix(sensor_position, end/2, end/2, :));
sd_3D_y.Iy = squeeze(sensor_data_3D_y(1).Iy(end/2, sensor_position, end/2, :));
sd_3D_z.Iz = squeeze(sensor_data_3D_z(1).Iz(end/2, end/2, sensor_position, :));
...
% intensity
error_intensity_1D_x = 100*abs(source_intensity - 1e-4*sd_1D_x.Ix_avg)/source_intensity;
error_intensity_2D_x = 100*abs(source_intensity - 1e-4*sd_2D_x.Ix_avg)/source_intensity;
error_intensity_2D_y = 100*abs(source_intensity - 1e-4*sd_2D_y.Iy_avg)/source_intensity;
error_intensity_3D_x = 100*abs(source_intensity - 1e-4*sd_3D_x.Ix_avg)/source_intensity;
error_intensity_3D_y = 100*abs(source_intensity - 1e-4*sd_3D_y.Iy_avg)/source_intensity;
error_intensity_3D_z = 100*abs(source_intensity - 1e-4*sd_3D_z.Iz_avg)/source_intensity;
...
if plot_comparisons
    figure;
    subplot(6, 1, 1), plot(sd_1D_x.Ix);
    title(['1D X, I_{avg} = ' num2str(sd_1D_x.Ix_avg*1e-4) ' W/cm^2']);
    subplot(6, 1, 2), plot(sd_2D_x.Ix);
    title(['2D X, I_{avg} = ' num2str(sd_2D_x.Ix_avg*1e-4) ' W/cm^2']);
    subplot(6, 1, 3), plot(sd_2D_y.Iy);
    title(['2D Y, I_{avg} = ' num2str(sd_2D_y.Iy_avg*1e-4) ' W/cm^2']);
    subplot(6, 1, 4), plot(sd_3D_x.Ix);
    title(['3D X, I_{avg} = ' num2str(sd_3D_x.Ix_avg*1e-4) ' W/cm^2']);
    subplot(6, 1, 5), plot(sd_3D_y.Iy);
    title(['3D Y, I_{avg} = ' num2str(sd_3D_y.Iy_avg*1e-4) ' W/cm^2']);
    subplot(6, 1, 6), plot(sd_3D_z.Iz);
    title(['3D Z, I_{avg} = ' num2str(sd_3D_z.Iz_avg*1e-4) ' W/cm^2']);
    scaleFig(1, 1.5);
    drawnow;
end